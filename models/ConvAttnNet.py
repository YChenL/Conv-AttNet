...
from models.attentions import WindowAttention
...
class LocalAttention(nn.Module):
    ...
    def __init__(self, dim_in, dim_out=None, *,
        ...
        self.attn = WindowAttention(dim=dim_in, window_size=[window_size, window_size], 
                                    num_heads=heads, attn_drop=dropout, proj_drop=dropout)
        self.window_size = window_size
        self.num_heads=heads
        # self.rel_index = self.rel_distance(window_size) + window_size - 1
        # self.rel_index[:, :, 0] *= 2 * window_size - 1
        # self.rel_index = self.rel_index.sum(-1)  # Wh*Ww, Wh*Ww
    ...
    def forward(self, x, mask=None):
        ...
        x = rearrange(x, "b c (n1 p1) (n2 p2) -> (b n1 n2) (p1 p2) c", p1=p, p2=p, n1=n1, n2=n2,)
...
def dnn_18(num_classes=1000, stem=True, name="ConvAttnNet_18", **block_kwargs):
    return ConvAttnNet(preresnet_dnn.BasicBlock, AttentionBasicBlockB, stem=partial(StemB, pool=stem),
                    num_blocks=(2, 2, 2, 2), num_blocks2=(0, 1, 1, 1), heads=(2, 4, 8, 16),
                    num_classes=num_classes, name=name, **block_kwargs)


def dnn_34(num_classes=1000, stem=True, name="ConvAttnNet_34", **block_kwargs):
    return ConvAttnNet(preresnet_dnn.BasicBlock, AttentionBasicBlockB, stem=partial(StemB, pool=stem),
                    num_blocks=(3, 4, 6, 4), num_blocks2=(0, 1, 3, 2), heads=(2, 4, 8, 16),
                    num_classes=num_classes, name=name, **block_kwargs)


def dnn_50(num_classes=1000, stem=True, name="ConvAttnNet_50", **block_kwargs):
    return ConvAttnNet(preresnet_dnn.Bottleneck, AttentionBlockB, stem=partial(StemB, pool=stem),
                    num_blocks=(3, 4, 6, 4), num_blocks2=(0, 1, 3, 2), heads=(4, 8, 16, 32),
                    num_classes=num_classes, name=name, **block_kwargs)


def dnn_101(num_classes=1000, stem=True, name="ConvAttnNet_101", **block_kwargs):
    return ConvAttnNet(preresnet_dnn.Bottleneck, AttentionBlockB, stem=partial(StemB, pool=stem),
                    num_blocks=(3, 4, 23, 4), num_blocks2=(0, 1, 3, 2), heads=(4, 8, 16, 32),
                    num_classes=num_classes, name=name, **block_kwargs)


def dnn_152(num_classes=1000, stem=True, name="ConvAttnNet_152", **block_kwargs):
    return ConvAttnNet(preresnet_dnn.Bottleneck, AttentionBlockB, stem=partial(StemB, pool=stem),
                    num_blocks=(3, 8, 36, 4), num_blocks2=(0, 1, 3, 2), heads=(4, 8, 16, 32),
                    num_classes=num_classes, name=name, **block_kwargs)
