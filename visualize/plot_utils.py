...
from visualize.utils import deprocess_image
...
def plot_gradcam(image, vgg_cam, res_cam, dense_cam):
    ...
    name_dict = {
        ...
        'GradCAM (ConvNext-34)': apply_mask(image, vgg_cam),
        'GradCAM (ResNet-34)': apply_mask(image, res_cam),
        'GradCAM (Conv-Attn-34)': apply_mask(image, dense_cam)
    }
    ...
    fig.suptitle(
        ...
        fontweight='bold', fontsize=16 #, dpi=320
    )
    plt.tight_layout()
