...
dirs = {
    'train': 'E:/datasets/imagenet-mini/train',
    'val': 'data/COVID-19_Radiography_Dataset/val',
    'test': 'E:/datasets/imagenet-mini/val'
}
...
def get_all_preds(model, loader):
    ...
    with torch.no_grad():
        ...
        all_CAMs = torch.tensor([], device=device)
        for batch in loader:
            images = batch[0].to(device)
            # preds, cam = model(images)
            preds = model(images)
            all_preds = torch.cat((all_preds, preds), dim=0)
            # all_CAMs = torch.cat((all_CAMs, cam), dim=0)
            
    return all_preds #, all_CAMs
